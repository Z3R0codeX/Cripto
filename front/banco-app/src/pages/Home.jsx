// src/pages/Home.jsx
import React, { useEffect } from 'react';
import { useBank } from '../context/BankContext';
import { Link } from 'react-router-dom';

export default function Home() {
    const { user, transactions, loadTransactions, contacts, loadContacts } = useBank();

    // Load data on mount
    useEffect(() => { loadContacts(); loadTransactions(); }, []);

    // Formatea el saldo del usuario
    const formattedBalance = user?.balance?.toLocaleString('en-US', {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    }) || '0.00';

    // Formatea las transacciones recientes para la tabla
    const recentTransactions = (transactions || []).slice(0, 5).map(tx => {
        const raw = Number(tx.MONTO || tx.monto || 0);
        let tipo = (tx.TIPO || tx.tipo || '').toLowerCase();
        
        let type = tipo.includes('out') ? 'Enviado' : (tipo.includes('in') ? 'Recibido' : 'Otro');
        const amount = tipo.includes('out') ? -Math.abs(raw) : Math.abs(raw);
        
        let name = tx.DESCRIPCION || (tx.wallet && tx.wallet.ID_WALLET) || 'Movimiento';
        
        try {
            const match = (contacts || []).find(c => {
                const desc = (tx.DESCRIPCION || '').toString();
                if (!desc) return false;
                if (c && (c.name === desc || c.account_number === desc || (tx.wallet?.ID_WALLET?.toString() === c.account_number))) return true;
                return false;
            });
            if (match) name = match.name;
        } catch (e) { /* ignore */ }
        
        const date = tx.created_at ? new Date(tx.created_at).toLocaleDateString() : 'N/A';
        
        return { id: tx.ID_TRANSACCION || tx.id, type, name, amount, date };
    });

    return (
        <div>
            <h2 style={{ marginBottom: '35px' }}>Resumen General</h2>
            
            <div className="dashboard-grid">
                
                {/* TARJETA 1: SALDO PRINCIPAL */}
                <div className="card balance-card">
                    <h3>Saldo Disponible</h3>
                    <div className="amount">${formattedBalance}</div> 
                    <p className="account-number">
                        Cuenta: **** {user?.account_number?.slice(-4) || '0000'}
                    </p>
                </div>

                {/* TARJETA 2: ACCIONES RÁPIDAS (SOLUCIÓN AL ANCHO) */}
                <div 
                    className="card actions-card"
                    // **CLAVE:** El estilo inline complementa el CSS para forzar la compresión
                    style={{ 
                        maxWidth: '480px', 
                        display: 'flex', 
                        flexDirection: 'column',
                        // Asegura que la tarjeta no se estire para llenar el espacio vacío del grid
                        justifySelf: 'flex-start' 
                    }}
                >
                    <h3>¿Qué quieres hacer?</h3>
                    <div className="actions-grid">
                        <Link to="transfer" className="action-btn">
                            <span className="icon">💸</span>
                            Transferir
                        </Link>
                        <Link to="contacts" className="action-btn">
                            <span className="icon">👥</span>
                            Contactos
                        </Link>
                        <Link to="profile" className="action-btn">
                            <span className="icon">⚙️</span>
                            Ajustes
                        </Link>
                    </div>
                </div>

                {/* TARJETA 3: ÚLTIMOS MOVIMIENTOS */}
                <div className="card history-card" style={{ gridColumn: '1 / -1', marginTop: '10px' }}>
                    <h3>Últimos Movimientos</h3>
                    
                    {recentTransactions.length > 0 ? (
                        <table className="contacts-table">
                            <thead>
                                <tr>
                                    <th>Concepto</th>
                                    <th>Fecha</th>
                                    <th style={{ textAlign: 'right' }}>Monto (USD)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {recentTransactions.map(t => {
                                    const isOut = t.amount < 0;
                                    // Usamos los colores de éxito/error, alineados a la derecha
                                    const amountColor = isOut ? '#e74c3c' : '#2ecc71'; 
                                    
                                    return (
                                        <tr key={t.id}>
                                            <td>
                                                <strong style={{ color: amountColor }}>
                                                    {t.type}
                                                </strong> - {t.name}
                                            </td>
                                            <td style={{ color: 'var(--text-muted)' }}>{t.date}</td>
                                            <td style={{ 
                                                textAlign: 'right', 
                                                fontWeight: 'bold',
                                                color: amountColor 
                                            }}>
                                                {isOut ? '-' : '+'}{Math.abs(t.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                                            </td>
                                        </tr>
                                    );
                                })}
                            </tbody>
                        </table>
                    ) : (
                        <p style={{ color: 'var(--text-muted)' }}>Aún no hay movimientos recientes.</p>
                    )}
                </div>
            </div>
        </div>
    );
}